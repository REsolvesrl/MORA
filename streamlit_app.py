...
with tab3:
    ...
    margine = st.slider(
        ...
        # ==========================================================
        # 📈 ANALISI ROI / MULTIPLO (pitch per co-investitori)
        # ==========================================================
        st.divider()
        st.markdown("#### 📈 Analisi di Redditività dell'Investimento")

        # --- Parametri di realizzo atteso ---
        roi1, roi2 = st.columns(2)
        perc_recupero = roi1.slider(
            "🎯 % di recupero atteso sul GBV (realizzo effettivo)",
            min_value=10, max_value=100, value=60, step=5,
            help="Quanto si stima di incassare realmente dal GBV tramite "
                 "asta, vendita o accordo. Es. 60% = si recupera € 60k ogni € 100k di GBV."
        )
        anni_realizzo = roi2.number_input(
            "⏳ Orizzonte temporale di realizzo (anni)",
            min_value=0.5, max_value=15.0, value=3.0, step=0.5,
            help="Tempo stimato per recuperare l'investimento (durata media procedura/vendita)."
        )

        # --- Calcolo ROI ---
        incasso_atteso = gbv_base * (perc_recupero / 100)
        # Capitale investito = offerta al servicer + tutti i costi sostenuti
        capitale_investito = offerta_target + spese_procedura + costi_acquisizione
        utile_netto = incasso_atteso - capitale_investito

        if capitale_investito > 0:
            roi_pct = (utile_netto / capitale_investito) * 100
            multiplo = incasso_atteso / capitale_investito
            roi_annuo = roi_pct / anni_realizzo if anni_realizzo > 0 else roi_pct
        else:
            roi_pct = multiplo = roi_annuo = 0.0

        # --- Metriche principali ---
        m1, m2, m3, m4 = st.columns(4)
        m1.metric(
            "💸 Capitale Investito",
            f"€ {capitale_investito:,.0f}",
            help="Offerta al Servicer + spese di procedura + costi di acquisizione."
        )
        m2.metric(
            "💰 Incasso Atteso",
            f"€ {incasso_atteso:,.0f}",
            help=f"{perc_recupero}% del GBV di € {gbv_base:,.0f}."
        )
        m3.metric(
            "📊 ROI complessivo",
            f"{roi_pct:,.1f}%",
            delta=f"{roi_annuo:,.1f}% / anno",
        )
        m4.metric(
            "✖️ Multiplo (MOIC)",
            f"{multiplo:,.2f}x",
            help="Multiple On Invested Capital: quanti euro incassi per ogni euro investito."
        )

        # --- Verdetto sull'investimento ---
        if multiplo >= 2.0:
            st.success(
                f"🚀 **Investimento molto interessante.** Con un esborso di "
                f"**€ {capitale_investito:,.0f}** si punta a incassare "
                f"**€ {incasso_atteso:,.0f}** in ~{anni_realizzo:g} anni: "
                f"un multiplo di **{multiplo:,.2f}x** (ROI **{roi_pct:,.1f}%**, "
                f"pari a **{roi_annuo:,.1f}%/anno**). "
                f"Utile netto atteso: **€ {utile_netto:,.0f}**."
            )
        elif multiplo >= 1.3:
            st.info(
                f"📈 **Investimento valido.** Multiplo **{multiplo:,.2f}x** "
                f"(ROI **{roi_pct:,.1f}%** su ~{anni_realizzo:g} anni → "
                f"**{roi_annuo:,.1f}%/anno**). Utile netto: **€ {utile_netto:,.0f}**. "
                f"Valutare il rischio-tempo e la solidità del realizzo atteso."
            )
        elif multiplo >= 1.0:
            st.warning(
                f"⚠️ **Margine risicato.** Multiplo solo **{multiplo:,.2f}x** "
                f"(utile **€ {utile_netto:,.0f}**). Il rischio procedura potrebbe "
                f"erodere il guadagno: rinegoziare l'offerta al ribasso o "
                f"rivedere la % di recupero attesa."
            )
        else:
            st.error(
                f"🛑 **Investimento in perdita.** A queste condizioni si incassa "
                f"meno di quanto investito (multiplo **{multiplo:,.2f}x**, "
                f"perdita **€ {abs(utile_netto):,.0f}**). "
                f"Abbassare l'offerta target o rivedere le ipotesi di recupero."
            )
